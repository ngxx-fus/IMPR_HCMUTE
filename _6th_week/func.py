...
def CMYK_splitter(img : Image) -> any:
    """
    One parameter of input with type PIL.image - RGB_mode
    Output is a tuple of 4 Images - 4 channels (CMYK) of input.
    """
    w, h = img.size
    C_channel = Image.new("RGB", (w, h), 0)
    M_channel = Image.new("RGB", (w, h), 0)
    Y_channel = Image.new("RGB", (w, h), 0)
    K_channel = Image.new("RGB", (w, h), 0)
    for x in range(w):
        for y in range(h):
            location = (x,y)
            ...
            M = (R, 0, B)
            Y = (R, G, 0)
            K = (min(R, G, B), min(R, G, B), min(R, G, B))
            C_channel.putpixel( location, C )
            M_channel.putpixel( location, M )
            Y_channel.putpixel( location, Y )
            K_channel.putpixel( location, K )
    return (C_channel, M_channel, Y_channel, K_channel)

def quick_show(message : str, image : Image)-> None:
    """
    Using CV2 to show PIL.Image.
    This function converts  PIL.Image -> np.array and reverse order of channels. (RGB->BGR)
    """
    np_image = np.array(image)
    BGR_image = cv2.cvtColor(np_image, cv2.COLOR_RGB2BGR)
    cv2.imshow(message, BGR_image )
